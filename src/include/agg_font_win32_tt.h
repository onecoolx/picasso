...
namespace agg
{
    ...
    public:
        ...
        void height(float h)         { m_height = agg::uround(h);  }
        void width(float w)          { m_width = agg::uround(w);   }
		...
		unsigned		units_per_em() const { return m_units_per_em; }
		...
		// this is picasso only function! not use it other place
		void 			release_dc0(void) { ::ReleaseDC(0, m_dc); }
    private:
