...
namespace agg
{
    ...
    public:
        ...
        //--------------------------------------------------------------------
        ~font_engine_freetype_base();
        font_engine_freetype_base(bool flag32, unsigned max_faces = MAX_FONTS);
        ...
        void resolution(unsigned dpi) { m_resolution = dpi; }
        void char_map(FT_Encoding map) { m_char_set = map; }
        void height(float h) { m_height = agg::uround(h * 64.0f); }
        void width(float w) { m_width = agg::uround(w * 64.0f); }
        ...
        float      height()       const { return float(m_height) / 64.0f;    }
        float      width()        const { return float(m_width) / 64.0f;     }
        ...
        float          advance_x()   const { return m_advance_x;   }
        float          advance_y()   const { return m_advance_y;   }
        void            write_glyph_to(int8u* data) const;
        bool            add_kerning(unsigned first, unsigned second,
                                    float* x, float* y);

		void set_antialias(bool b) { m_antialias = b; }
    ...
    public:
        ...
        font_engine_freetype_int16(unsigned max_faces = MAX_FONTS) : 
    ...
    public:
        ...
        font_engine_freetype_int32(unsigned max_faces = MAX_FONTS) : 
