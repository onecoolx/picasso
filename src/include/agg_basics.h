...
#pragma warning(disable : 4035) //Disable warning "no return value"
    AGG_INLINE int iround(float v)              //-------iround
    {
        int t;
    ...
    AGG_INLINE unsigned uround(float v)         //-------uround
    {
        unsigned t;
        __asm fld   qword ptr [v]
        __asm fistp dword ptr [t]
...
#pragma warning(pop)
    AGG_INLINE unsigned ufloor(float v)         //-------ufloor
    {
        return unsigned(floor(v));
    }
    AGG_INLINE unsigned uceil(float v)          //--------uceil
    {
        return unsigned(ceil(v));
    }
#elif defined(AGG_QIFIST)
    AGG_INLINE int iround(float v)
    {
        return int(v);
    }
    AGG_INLINE int uround(float v)
    {
        return unsigned(v);
    }
    AGG_INLINE unsigned ufloor(float v)
    {
        return unsigned(floor(v));
    }
    AGG_INLINE unsigned uceil(float v)
    {
        return unsigned(ceil(v));
    }
#else
    ...
    {
        AGG_INLINE static int iround(float v)
        {
    ...
    typedef rect_base<float> rect_d; //----rect_d
    ...
    typedef point_base<float> point_d; //-----point_d
    ...
    typedef vertex_base<float> vertex_d; //-----vertex_d
    ...
    {
        return fabs(v1 - v2) <= float(epsilon);
    }
