...
namespace agg
{
    ...
    public:
        ...
        line_image_scale(const Source& src, float height) :
            m_source(src), 
            m_height(height),
        ...
        float width()  const { return m_source.width(); }
        float height() const { return m_height; }

        color_type pixel(int x, int y) const 
        { 
            float src_y = (y + 0.5) * m_scale - 0.5;
    ...
    public:
        ...
        float width()      const { return m_height; }
    ...
    public:
        ...
        distance_interpolator4(int x1,  int y1, int x2, int y2,
                               ...
                               int len, float scale, int x, int y) :
            m_dx(x2 - x1),
            m_dy(y2 - y1),
            ...
            m_dist(iround(float(x + line_subpixel_scale/2 - x2) * float(m_dy) - 
                          float(y + line_subpixel_scale/2 - y2) * float(m_dx))),
    ...
    public:
        ...
        //---------------------------------------------------------------------
        void   scale_x(float s) { m_scale_x = s; }
        float scale_x() const   { return m_scale_x; }

        //---------------------------------------------------------------------
        void   start_x(float s) { m_start = iround(s * line_subpixel_scale); }
        float start_x() const   { return float(m_start) / line_subpixel_scale; }

        //---------------------------------------------------------------------
        int subpixel_width() const { return m_pattern->line_width(); }
        int pattern_width() const { return m_pattern->pattern_width(); }
        float width() const { return float(subpixel_width()) / line_subpixel_scale; }
