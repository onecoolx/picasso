...
namespace agg
{
    ...
    template<class T, unsigned CoordShift=6> struct vertex_integer
    {
        ...
        unsigned vertex(float* x_, float* y_, 
                        float dx=0, float dy=0,
                        float scale=1.0) const
        {
            *x_ = dx + (float(x >> 1) / coord_scale) * scale;
            *y_ = dy + (float(y >> 1) / coord_scale) * scale;
            switch(((y & 1) << 1) | (x & 1))
            {
    ...
    public:
        ...
        {
            rect_d bounds(1e29f, 1e29f, -1e29f, -1e29f);
            if(m_storage.size() == 0)
            {
    ...
    public:
        ...
        void init(const int8u* data, unsigned size, 
                  float dx, float dy, float scale=1.0)
        {
            m_data     = data;
            m_end      = data + size;
            m_ptr      = data;
