...
namespace agg
{
    ...
    struct matrix_pivot
    {
        static int pivot(float m[Rows][Cols], unsigned row)
        {
            int k = int(row);
            float max_val, tmp;
    ...
    struct simul_eq
    {
        static bool solve(const float left[Size][Size], 
                          const float right[Size][RightCols],
                          float result[Size][RightCols])
        {
            unsigned i, j, k;
            float a1;

            float tmp[Size][Size + RightCols];
