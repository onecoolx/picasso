...
namespace agg
{
    ...
    struct conv_clip_polyline : public conv_adaptor_vpgen<VertexSource, vpgen_clip_polyline>
    {
        ...
        float x1() const { return base_type::vpgen().x1(); }
        float y1() const { return base_type::vpgen().y1(); }
        float x2() const { return base_type::vpgen().x2(); }
        float y2() const { return base_type::vpgen().y2(); }
