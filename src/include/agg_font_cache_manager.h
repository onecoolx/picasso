...
namespace agg
{
    ...
    public:
                                 ...
                                 float          advance_y)
        {
            ...
            {
                m_glyphs[msb] = 
                    (glyph_cache**)m_allocator.allocate(sizeof(glyph_cache*) * MAX_CACHE, 
                                                        sizeof(glyph_cache*));
                memset(m_glyphs[msb], 0, sizeof(glyph_cache*) * MAX_CACHE);
            }
    ...
    private:
        ...
        glyph_cache**   m_glyphs[MAX_CACHE];
    ...
    public:
        ...
        //--------------------------------------------------------------------
        font_cache_pool(unsigned max_fonts=MAX_FONTS) : 
    ...
    public:
        ...
        //--------------------------------------------------------------------
        font_cache_manager(unsigned max_fonts=MAX_FONTS) :
            m_fonts(max_fonts),
            m_engine(0),
            m_change_stamp(-1),
            m_prev_glyph(0),
		...
		void attach (font_engine_type& engine) //picasso add to two step create
		{
			m_engine = &engine;
		}
        font_cache_manager(font_engine_type& engine, unsigned max_fonts=MAX_FONTS) :
            m_fonts(max_fonts),
            m_engine(&engine),
            m_change_stamp(-1),
            m_prev_glyph(0),
        ...
        const glyph_cache* glyph(unsigned glyph_code)
        {
            ...
            {
                if(m_engine && m_engine->prepare_glyph(glyph_code))
                {
                    m_prev_glyph = m_last_glyph;
        ...
        //--------------------------------------------------------------------
        bool add_kerning(float* x, float* y)
        {
            if(m_prev_glyph && m_last_glyph && m_engine)
            {
    ...
    private:
        ...
        void synchronize()
        {
            if(m_engine && (m_change_stamp != m_engine->change_stamp()))
            {
