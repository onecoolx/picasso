...
namespace agg
{
    ...
    //---------------------------------------------------------------------
    void line_profile_aa::width(float w)
    {
        if(w < 0.0) w = 0.0;
    ...
    //---------------------------------------------------------------------
    line_profile_aa::value_type* line_profile_aa::profile(float w)
    {
    ...
    //---------------------------------------------------------------------
    void line_profile_aa::set(float center_width, float smoother_width)
    {
        float base_val = 1.0;
        if(center_width == 0.0)   center_width = 1.0 / subpixel_scale;
        if(smoother_width == 0.0) smoother_width = 1.0 / subpixel_scale;

        float width = center_width + smoother_width;
        if(width < m_min_width)
        {
            float k = width / m_min_width;
            base_val *= k;
            center_width /= k;
            smoother_width /= k;
        }
        ...
        for(i = 0; i < subpixel_smoother_width; i++)
        {
            *ch_smoother++ = 
                m_gamma[unsigned((base_val - 
                                  base_val * 
                                  (float(i) / subpixel_smoother_width)) * aa_mask)];
        }
