...
namespace agg
{
    ...
    public:
        ...
        void clear(const color_type& c)
        {
            ...
            if(w)
            {
                for(unsigned i = y; i < y2+1; i++)
                {
                    m_ren->copy_hline(x, i, w, c);
                }
            }
        }

        void clear_box(const color_type&c, int x, int y, int w, int h)
        {
            if (w && h) {
                if (x < 0) x = 0;
                if (y < 0) y = 0;
                int max_h = (y+h > (int)height()) ? ((int)height()-y) : h;
                int max_w = (x+w > (int)width()) ? ((int)width()-x) : w;
                for (int i = 0; i < max_h; i++) {
                    m_ren->copy_hline(x, y+i, max_w, c);
                }
            }
        }
        ...
        void copy_absolute_from(const RenBuf& src, 
