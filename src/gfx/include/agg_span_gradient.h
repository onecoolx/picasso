...
namespace agg
{
    ...
    public:
        ...
        int calculate(int x, int y, int) const
        {
            ...
            return iround((dx * m_fx + dy * m_fy + (float)sqrt((float)fabs(d3))) * m_mul);
        }
    ...
    public:
        ...
        static AGG_INLINE int calculate(int x, int y, int d) 
        { 
            return uround((float)fabs((float)atan2(float(y), float(x))) * float(d) / pi);
        }
    };

    //=================================================gradient_once_adaptor
    template<class GradientF> class gradient_pad_adaptor
    {
    public:
        gradient_pad_adaptor(const GradientF& gradient) : 
            m_gradient(&gradient) {}
