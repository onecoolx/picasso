...
static fixed_type cos_table[512] =
{
...
static fixed_type tan_table[256] =
{
...
static fixed_type acos_table[513] =
{
...
static unsigned short sqrt_table[256] =
{
...
// PI / 256 
static fixed pid256(3.14159265358979323846 / 256);
// PI / 128
static fixed pid128(3.14159265358979323846 / 128);
// 256 / PI
static fixed pi256(256 / 3.14159265358979323846);
// 1 / 2PI
static fixed pi12(1/(3.14159265358979323846*2));
// 1 / PI
static fixed pi1(1/(3.14159265358979323846));
// -1
static fixed nav_1(-1);
// 1
static fixed num_1(1);

// PI
static fixed pi(3.14159265358979323846);
...
// 2PI
static fixed pi2(3.14159265358979323846 * 2);
...
// PI/2
static fixed pid2(3.14159265358979323846 / 2);
...
fixed atan(fixed r)
{
   fixed_type a, b, c;            /* for binary search */
   fixed d;                /* difference value for search */

   if (r >= fixed::fixed_0()) {           /* search the first part of tan table */
      a = 0;
      b = 127;
...
{
    fixed_type dx, xx;
    fixed_type cx = 0;             /* if no bit set: default %cl = 2n = 0 */
    ...
    dx = x.data() >> 6;
    for (fixed_type i = 0; i < (fixed_type)FIXED_BITS; i++) {
        if ((dx << i) & FIXED_MIN) {
            cx = FIXED_BITS - i;
            break;
        }
    ...
    return fixed(xx >> 4, 0);          /* adjust the result */
}
