...
void PICAPI ps_path_line_to(ps_path* path, const ps_point* p)
{
    ...
    path->path.line_to(DBL_TO_SCALAR(p->x), DBL_TO_SCALAR(p->y));
    global_status = STATUS_SUCCEED;
}
...
void PICAPI ps_path_tangent_arc_to(ps_path* path, double r, const ps_point* tp, const ps_point* ep)
{
    ...
    double tangent = r / tan(acos(cos_phi) / 2);
    ...
    double factor_ra = r / orth_p1p0_length;
...
void PICAPI ps_path_bezier_to(ps_path* path, const ps_point* cp1, const ps_point* cp2, const ps_point* ep)
{
    ...
    picasso::curve4 c(path->path.last_x(), path->path.last_y(), DBL_TO_SCALAR(cp1->x), DBL_TO_SCALAR(cp1->y), 
                    DBL_TO_SCALAR(cp2->x), DBL_TO_SCALAR(cp2->y), DBL_TO_SCALAR(ep->x), DBL_TO_SCALAR(ep->y));
    if (_is_closed_path(path->path))
...
void PICAPI ps_path_quad_to(ps_path* path, const ps_point* cp, const ps_point* ep)
{
    ...
    picasso::curve3 c(path->path.last_x(), path->path.last_y(), DBL_TO_SCALAR(cp->x), 
                    DBL_TO_SCALAR(cp->y), DBL_TO_SCALAR(ep->x), DBL_TO_SCALAR(ep->y));
...
double PICAPI ps_path_get_length(const ps_path* path)
{
    ...
    global_status = STATUS_SUCCEED;
    return SCALAR_TO_DBL(picasso::path_length(const_cast<ps_path*>(path)->path, 0));
}
...
ps_bool PICAPI ps_path_contains(const ps_path* path, const ps_point* p, ps_fill_rule rule)
{
    ...
    return picasso::raster_adapter::fill_contents_point(path->path, 
                DBL_TO_SCALAR(p->x), DBL_TO_SCALAR(p->y), (picasso::filling_rule)rule) ? True : False;
}
...
void PICAPI ps_path_add_line(ps_path* path, const ps_point* p1, const ps_point* p2)
{
    ...
    path->path.move_to(DBL_TO_SCALAR(p1->x), DBL_TO_SCALAR(p1->y));
    path->path.line_to(DBL_TO_SCALAR(p2->x), DBL_TO_SCALAR(p2->y));
    global_status = STATUS_SUCCEED;
}
...
void PICAPI ps_path_add_arc(ps_path* path, const ps_point* cp, double r, double sa, double ea, ps_bool cw)
{
    ...
    picasso::arc a(DBL_TO_SCALAR(cp->x), DBL_TO_SCALAR(cp->y), DBL_TO_SCALAR(r), DBL_TO_SCALAR(r), 
                DBL_TO_SCALAR(sa), DBL_TO_SCALAR(ea), (cw ? true : false));
...
void PICAPI ps_path_add_rect(ps_path* path, const ps_rect* r)
{
    ...
    path->path.move_to(DBL_TO_SCALAR(r->x), DBL_TO_SCALAR(r->y));
    path->path.hline_rel(DBL_TO_SCALAR(r->w));
    path->path.vline_rel(DBL_TO_SCALAR(r->h));
    path->path.hline_rel(-DBL_TO_SCALAR(r->w));
    path->path.end_poly();
    global_status = STATUS_SUCCEED;
}
...
void PICAPI ps_path_add_ellipse(ps_path* path, const ps_rect* r)
{
    ...
    picasso::ellipse e(DBL_TO_SCALAR(r->x+r->w/2), DBL_TO_SCALAR(r->y+r->h/2), 
