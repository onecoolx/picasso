...
namespace agg
{
    ...
    unsigned arc::vertex(float* x, float* y)
    {
        ...
        {
            *x = m_x + (float)cos(m_end) * m_rx;
            *y = m_y + (float)sin(m_end) * m_ry;
            m_path_cmd = path_cmd_stop;
            return path_cmd_line_to;
        }

        *x = m_x + (float)cos(m_angle) * m_rx;
        *y = m_y + (float)sin(m_angle) * m_ry;
    ...
    void arc::normalize(float a1, float a2, bool ccw)
    {
        ...
        m_da = (float)acos(ra / (ra + 0.125f / m_scale)) * 2;
        if(ccw)
        {
