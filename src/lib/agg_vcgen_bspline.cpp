...
namespace agg
{
    ...
    vcgen_bspline::vcgen_bspline() :
        ...
        m_spline_x(),
        m_spline_y(),
        m_interpolation_step(1.0f/50.0f),
        m_closed(0),
        m_status(initial),
        m_src_vertex(0)
    {
    ...
    //------------------------------------------------------------------------
    void vcgen_bspline::add_vertex(float x, float y, unsigned cmd)
    {
        m_status = initial;
        if(is_move_to(cmd))
        {
    ...
    void vcgen_bspline::rewind(unsigned)
    {
        ...
        if(m_status == initial && m_src_vertices.size() > 2)
        {
            ...
            {
                float x = float(m_closed ? i + 4 : i);
                m_spline_x.add_point(x, m_src_vertices[i].x);
                m_spline_y.add_point(x, m_src_vertices[i].y);
            }
            m_cur_abscissa = 0.0f;
            m_max_abscissa = float(m_src_vertices.size() - 1);
            if(m_closed)
            {
                m_cur_abscissa = 4.0f;
                m_max_abscissa += 5.0f;
                m_spline_x.add_point(float(m_src_vertices.size() + 4), float(m_src_vertices[0].x));
                m_spline_y.add_point(float(m_src_vertices.size() + 4), float(m_src_vertices[0].y));
                m_spline_x.add_point(float(m_src_vertices.size() + 5), float(m_src_vertices[1].x));
                m_spline_y.add_point(float(m_src_vertices.size() + 5), float(m_src_vertices[1].y));
                m_spline_x.add_point(float(m_src_vertices.size() + 6), float(m_src_vertices[2].x));
                m_spline_y.add_point(float(m_src_vertices.size() + 6), float(m_src_vertices[2].y));
                m_spline_x.add_point(float(m_src_vertices.size() + 7), float(m_src_vertices.next(2).x));
                m_spline_y.add_point(float(m_src_vertices.size() + 7), float(m_src_vertices.next(2).y));
            }
            m_spline_x.prepare();
            m_spline_y.prepare();
        }
    ...
    //------------------------------------------------------------------------
    unsigned vcgen_bspline::vertex(float* x, float* y)
    {
