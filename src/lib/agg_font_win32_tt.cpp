...
namespace agg
{
    ...
    //------------------------------------------------------------------------
    static inline FIXED dbl_to_fx(float d)
    {
        int l;
        l = int(d * 65536.0);
        return *(FIXED*)(&l);
    ...
    //------------------------------------------------------------------------
    static inline float fx_to_dbl(const FIXED& p)
    {
                                                ...
                                                glyph_rendering ren_type)
    {
        if(m_dc)
        {
            ...
            if(idx >= 0)
            {
				...
				GetOutlineTextMetrics(m_dc, sizeof(omt), &omt);
				TEXTMETRIC& mt = omt.otmTextMetrics;
				m_ascent = mt.tmAscent;
				m_descent = mt.tmDescent;
				m_leading = mt.tmExternalLeading;
				m_units_per_em = omt.otmEMSquare;
                                                ...
                                                float* x, float* y)
    {
        if(m_dc && m_cur_font)
        {
            ...
            while(beg <= end)
            {
                ...
                {
                    float dx = m_kerning_pairs[mid].iKernAmount;
                    float dy = 0.0f;
                    if(m_glyph_rendering == glyph_ren_outline ||
                       m_glyph_rendering == glyph_ren_agg_mono ||
                       m_glyph_rendering == glyph_ren_agg_gray8)
