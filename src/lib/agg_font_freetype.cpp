...
namespace agg
{
    ...
    //------------------------------------------------------------------------
    static inline int dbl_to_plain_fx(float d)
    {
        return int(d * 65536.0f);
    }

    //------------------------------------------------------------------------
    static inline float int26p6_to_dbl(int p)
    {
        return float(p) / 64.0f;
    }

    //------------------------------------------------------------------------
    static inline int dbl_to_int26p6(float p)
    {
        return int(p * 64.0f + 0.5f);
    }
    ...
    //------------------------------------------------------------------------
    float font_engine_freetype_base::ascender() const
    {
        if(m_cur_face)
        {
    ...
    //------------------------------------------------------------------------
    float font_engine_freetype_base::descender() const
    {
        if(m_cur_face)
        {
	...
	float font_engine_freetype_base::leading() const
	{
        if(m_cur_face)
        {
            return (float)(m_cur_face->height - m_cur_face->ascender + m_cur_face->descender) / 64.0f;
        }
        return 0.0f;
	}

	unsigned font_engine_freetype_base::units_per_em() const
	{
        if(m_cur_face)
        {
            return m_cur_face->units_per_EM;
        }
        return 0;
	}
                                              ...
                                              const long font_mem_size)
    {
        ...
        if(m_library_initialized)
        {
            ...
            unsigned name_len = strlen(font_name);
            if(name_len > m_name_len)
            {
                delete [] m_signature;
				delete [] m_name;
                m_name  = new char [name_len + 32];
                m_signature = new char [name_len + 32 + 256];
                m_name_len = name_len + 32 - 1;
            }

            strcpy(m_name, font_name);

            update_signature();
            int idx = find_face(m_signature);
            if(idx >= 0)
            {
            ...
            else
            {
                ...
                if(m_last_error == 0)
                {
                    m_face_names[m_num_faces] = new char [strlen(m_signature) + 1];
                    strcpy(m_face_names[m_num_faces], m_signature);
                    m_cur_face = m_faces[m_num_faces];
            		update_char_size();
            		FT_Select_Charmap(m_cur_face, m_char_map);
                    ++m_num_faces;
                }
                else
                {
    ...
    void font_engine_freetype_base::transform(const trans_affine& affine)
    {
        m_affine = affine;
		if (m_italic)
			m_affine *= trans_affine_skewing(-0.4, 0 );
    }
...
//
		glyph_rendering old = m_glyph_rendering;
		if (m_cur_face->glyph->format == FT_GLYPH_FORMAT_BITMAP)
			m_glyph_rendering = glyph_ren_native_mono; //bitmap is mono 
        ...
        if(m_last_error == 0)
        {
            ...
            case glyph_ren_outline:
                if(m_last_error == 0)
                {
					if (m_weight == 500) {
						int strength = 1 << 5;
						FT_Outline_Embolden(&(m_cur_face->glyph->outline), strength);
					} else if (m_weight == 700) {
						int strength = 1 << 6;
						FT_Outline_Embolden(&(m_cur_face->glyph->outline), strength);
					} else if (m_weight == 900) {
						int strength = 1 << 7;
						FT_Outline_Embolden(&(m_cur_face->glyph->outline), strength);
					}
                                           ...
                                           float* x, float* y)
    {
        if(m_cur_face && first && second && FT_HAS_KERNING(m_cur_face))
        {
            ...
            float dx = int26p6_to_dbl(delta.x);
            float dy = int26p6_to_dbl(delta.y);
            if(m_glyph_rendering == glyph_ren_outline ||
               m_glyph_rendering == glyph_ren_agg_mono ||
               m_glyph_rendering == glyph_ren_agg_gray8)
            {
