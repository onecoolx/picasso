...
> 版本: 2.9.0 | 许可证: BSD 2-Clause  
> 本指南按 API 分组介绍 Picasso 编程方法，每个示例均附有 SVG 模拟的显示效果。

---

## 目录

1. [初始化与生命周期](#1-initialization--lifecycle)
2. [画布](#2-canvas)
3. [上下文](#3-context)
4. [路径](#4-path)
5. [绘制操作](#5-drawing-operations)
6. [描边样式](#6-stroke-style)
7. [填充源](#7-fill-source)
8. [渐变](#8-gradient)
9. [图案](#9-pattern)
10. [矩阵变换](#10-matrix-transform)
11. [裁剪](#11-clipping)
12. [图像](#12-image)
13. [蒙版](#13-mask)
14. [字体与文本](#14-font--text)
15. [扩展：图像加载](#15-extension-image-loading)
16. [扩展：SVG 渲染](#16-extension-svg-rendering)
17. [扩展：SVG 动画](#17-extension-svg-animation)

---

## 1. 初始化与生命周期

使用 Picasso 前必须初始化，使用完毕后必须关闭。所有 API 调用必须在 `ps_initialize()` and `ps_shutdown()`.
...
画布是 Picasso 的渲染目标 — 本质上是一个像素缓冲区。所有绘制操作最终都写入画布。
...
上下文是 Picasso 的核心对象，持有绘制状态栋 (transform matrix, pen, brush, clipping region, etc.).
...
Picasso 提供四种基本绘制操作，作用于上下文中的当前路径。
...
Picasso 支持三种渐变类型：线性渐变、径向渐变和锥形渐变。
...
SVG 扩展提供 SVG Tiny 1.2 文档的解析和渲染功能。
...
> 本编程指南基于 Picasso v2.9.0 API。完整 API 参考请访问：
