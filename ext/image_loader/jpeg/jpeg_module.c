...
static int decode_jpg_data(psx_image_header* header, int idx, ps_byte* buffer, size_t buffer_len)
{
    int y; size_t i;
...
static int encode_jpg_data(psx_image_header* header, int idx, const ps_byte* buffer, size_t buffer_len, int* ret)
{
    int y;
    struct jpeg_image_ctx* ctx = (struct jpeg_image_ctx*)header->priv;
